/* eslint-disable @next/next/no-img-element */
import Image from "next/image";
import { title } from "process";
import React from "react";

export const Technos = () => {
    const technos = [
        {
            title: "Skilled Technos",
            technologies: [
                {
                    name: "React",
                    icon: <svg width="101" height="100" viewBox="0 0 101 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M50.3067 58.8168C55.1757 58.8168 59.1229 54.8697 59.1229 50.0006C59.1229 45.1315 55.1757 41.1844 50.3067 41.1844C45.4376 41.1844 41.4904 45.1315 41.4904 50.0006C41.4904 54.8697 45.4376 58.8168 50.3067 58.8168Z" fill="#61DAFB" />
                        <path d="M50.3066 68.0631C76.4333 68.0631 97.6132 59.9762 97.6132 50.0005C97.6132 40.0249 76.4333 31.938 50.3066 31.938C24.1799 31.938 3 40.0249 3 50.0005C3 59.9762 24.1799 68.0631 50.3066 68.0631Z" stroke="#61DAFB" stroke-width="5" />
                        <path d="M34.664 59.0318C47.7274 81.6582 65.3207 95.9571 73.9599 90.9692C82.5991 85.9814 79.0126 63.5957 65.9492 40.9693C52.8858 18.3428 35.2925 4.04395 26.6533 9.03178C18.0141 14.0196 21.6006 36.4054 34.664 59.0318Z" stroke="#61DAFB" stroke-width="5" />
                        <path d="M34.664 40.9692C21.6007 63.5956 18.0141 85.9814 26.6533 90.9692C35.2925 95.957 52.8859 81.6582 65.9492 59.0318C79.0126 36.4053 82.5991 14.0196 73.9599 9.03176C65.3208 4.04393 47.7274 18.3428 34.664 40.9692Z" stroke="#61DAFB" stroke-width="5" />
                    </svg>

                },
                {
                    name: "TypeScript",
                    icon: <svg width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 0.000488281H100V100H0V0.000488281Z" fill="#017ACB" />
                        <path d="M48.0157 37.0317H52.8126V45.313H39.8438V82.1567L39.5 82.2505C39.0313 82.3755 32.8594 82.3755 31.5313 82.2349L30.4688 82.1411V45.313H17.5V37.0317L21.6094 36.9849C23.8594 36.9536 29.6406 36.9536 34.4532 36.9849C39.2657 37.0161 45.3594 37.0317 48.0157 37.0317ZM84.6251 78.2505C82.7189 80.2661 80.672 81.3911 77.2657 82.313C75.7813 82.7192 75.5313 82.7349 72.1876 82.7192C68.8438 82.7036 68.5782 82.7036 66.9532 82.2817C62.7501 81.2036 59.3594 79.0942 57.0469 76.1099C56.3907 75.2661 55.3126 73.5161 55.3126 73.2974C55.3126 73.2349 55.4688 73.0942 55.6719 73.0005C55.8751 72.9067 56.2969 72.6411 56.6407 72.438C56.9844 72.2349 57.6094 71.8599 58.0313 71.6411C58.4532 71.4224 59.6719 70.7036 60.7345 70.063C61.797 69.4224 62.7657 68.9067 62.8751 68.9067C62.9845 68.9067 63.1876 69.1255 63.3438 69.3911C64.2813 70.9692 66.4688 72.9849 68.0157 73.6724C68.9688 74.0786 71.0782 74.5317 72.0938 74.5317C73.0313 74.5317 74.7501 74.1255 75.672 73.7036C76.6563 73.2505 77.1564 72.7974 77.7501 71.8911C78.1564 71.2505 78.2032 71.0786 78.1876 69.8599C78.1876 68.7349 78.1251 68.4224 77.8126 67.9067C76.9376 66.4692 75.7501 65.7192 70.9376 63.5942C65.9688 61.3911 63.7345 60.0786 61.9219 58.313C60.5782 57.0005 60.3126 56.6411 59.4688 55.0005C58.3751 52.8911 58.2344 52.2036 58.2188 49.063C58.2032 46.8599 58.2501 46.1411 58.4844 45.3911C58.8126 44.2661 59.8751 42.0942 60.3594 41.5474C61.3594 40.3755 61.7188 40.0161 62.422 39.438C64.547 37.688 67.8595 36.5317 71.0313 36.4224C71.3907 36.4224 72.5782 36.4849 73.6876 36.563C76.8751 36.8286 79.047 37.6099 81.1407 39.2817C82.7189 40.5317 85.1095 43.4692 84.8751 43.8599C84.7189 44.0942 78.4845 48.2505 78.0782 48.3755C77.8282 48.4536 77.6564 48.3599 77.3126 47.9536C75.1876 45.4067 74.3282 44.8599 72.2657 44.7349C70.797 44.6411 70.0157 44.813 69.0313 45.4692C68.0001 46.1567 67.5001 47.2036 67.5001 48.6567C67.5157 50.7817 68.3282 51.7817 71.3282 53.2661C73.2657 54.2192 74.922 55.0005 75.047 55.0005C75.2345 55.0005 79.2501 57.0005 80.297 57.6255C85.172 60.4849 87.1564 63.4224 87.672 68.4849C88.047 72.2974 86.9689 75.7817 84.6251 78.2505Z" fill="white" />
                    </svg>
                },
                {
                    name: "Next.js",
                    icon: <svg width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M50 99.9996C77.6142 99.9996 100 77.6139 100 49.9996C100 22.3854 77.6142 -0.000366211 50 -0.000366211C22.3858 -0.000366211 0 22.3854 0 49.9996C0 77.6139 22.3858 99.9996 50 99.9996Z" fill="black" />
                        <path d="M83.06 87.511L38.4122 29.9996H30V69.983H36.7298V38.546L77.7773 91.5803C79.6294 90.3405 81.394 88.9804 83.06 87.511Z" fill="url(#paint0_linear_790_2962)" />
                        <path d="M70.5558 29.9996H63.8892V69.9996H70.5558V29.9996Z" fill="url(#paint1_linear_790_2962)" />
                        <defs>
                            <linearGradient id="paint0_linear_790_2962" x1="60.5555" y1="64.7219" x2="80.2778" y2="89.1663" gradientUnits="userSpaceOnUse">
                                <stop stop-color="white" />
                                <stop offset="1" stop-color="white" stop-opacity="0" />
                            </linearGradient>
                            <linearGradient id="paint1_linear_790_2962" x1="67.2225" y1="29.9996" x2="67.1109" y2="59.3748" gradientUnits="userSpaceOnUse">
                                <stop stop-color="white" />
                                <stop offset="1" stop-color="white" stop-opacity="0" />
                            </linearGradient>
                        </defs>
                    </svg>
                },
                {
                    name: "Bootstrap",
                    icon: <svg width="100" height="80" viewBox="0 0 100 80" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M20.7703 0C15.0645 0 10.8424 4.99414 11.0314 10.4102C11.2129 15.6135 10.9771 22.3525 9.28066 27.8482C7.57812 33.3594 4.70078 36.8518 0 37.3V42.3609C4.70078 42.8102 7.57812 46.3008 9.28047 51.8127C10.9771 57.3084 11.2127 64.0475 11.0312 69.2508C10.8422 74.666 15.0643 79.6609 20.7711 79.6609H79.2373C84.9432 79.6609 89.1645 74.6668 88.9754 69.2508C88.7939 64.0475 89.0297 57.3084 90.7262 51.8127C92.4287 46.3008 95.299 42.8088 100 42.3609V37.3C95.2992 36.8508 92.4289 33.3602 90.7262 27.8482C89.0295 22.3533 88.7939 15.6135 88.9754 10.4102C89.1645 4.99492 84.9432 0 79.2373 0H20.7695H20.7703ZM67.7967 49.0365C67.7967 56.4941 62.234 61.017 53.0025 61.017H37.2881C36.8386 61.017 36.4075 60.8384 36.0896 60.5206C35.7717 60.2027 35.5932 59.7716 35.5932 59.3221V20.3391C35.5932 19.8895 35.7717 19.4584 36.0896 19.1406C36.4075 18.8227 36.8386 18.6441 37.2881 18.6441H52.9131C60.6105 18.6441 65.6623 22.8137 65.6623 29.2152C65.6623 33.7084 62.2639 37.7309 57.9344 38.4355V38.6699C63.8281 39.3164 67.7967 43.3984 67.7967 49.0365ZM51.2891 24.017H42.3289V36.6732H49.8756C55.7096 36.6732 58.9264 34.324 58.9264 30.125C58.9258 26.1898 56.1602 24.017 51.2891 24.017ZM42.3289 41.6949V55.6424H51.6187C57.693 55.6424 60.9104 53.2051 60.9104 48.6246C60.9104 44.0441 57.6027 41.6941 51.2281 41.6941H42.3289V41.6949Z" fill="#7952B3" />
                    </svg>

                },
                {
                    name: "Node.js",
                    icon: <svg width="91" height="100" viewBox="0 0 91 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M41.2785 1.06735C43.758 -0.351924 46.9876 -0.358335 49.465 1.06735C61.9286 8.10993 74.3961 15.1415 86.8576 22.1883C89.2014 23.5086 90.7692 26.1171 90.7454 28.8178V71.1886C90.7628 74.0012 89.0401 76.672 86.5673 77.9623C74.1447 84.9661 61.7285 91.9784 49.308 98.9821C46.7772 100.429 43.4827 100.318 41.031 98.7521C37.3067 96.5931 33.576 94.4447 29.8513 92.2879C29.0902 91.8341 28.2322 91.473 27.6948 90.735C28.1699 90.0947 29.0193 90.0149 29.7096 89.7353C31.2642 89.241 32.692 88.4475 34.1202 87.6775C34.4813 87.4303 34.9222 87.525 35.2684 87.7462C38.4531 89.5722 41.6097 91.4513 44.8051 93.2598C45.4868 93.6534 46.177 93.1309 46.7597 92.8061C58.9501 85.9163 71.1555 79.0525 83.3438 72.1606C83.7954 71.9433 84.0451 71.4639 84.0084 70.9692C84.0169 56.992 84.0105 43.0123 84.0126 29.035C84.0643 28.4737 83.7395 27.9576 83.232 27.7279C70.8524 20.7562 58.4793 13.7738 46.1019 6.80035C45.8874 6.65292 45.6334 6.57382 45.3732 6.57344C45.1129 6.57306 44.8587 6.65141 44.6438 6.79821C32.2664 13.7738 19.8954 20.7626 7.51799 27.734C7.01226 27.9641 6.67284 28.4716 6.73303 29.035C6.73517 43.0123 6.73303 56.992 6.73303 70.9714C6.71157 71.2127 6.76458 71.4549 6.88493 71.6652C7.00528 71.8756 7.18719 72.044 7.40616 72.1478C10.7091 74.0208 14.0164 75.8806 17.3215 77.7472C19.1834 78.7494 21.4696 79.3449 23.521 78.577C25.3314 77.9278 26.6003 76.0804 26.5658 74.1582C26.5829 60.2626 26.5573 46.3647 26.5786 32.4712C26.5334 31.8543 27.1185 31.3447 27.7183 31.4027C29.3053 31.392 30.8945 31.3814 32.4815 31.407C33.144 31.392 33.5998 32.0563 33.5179 32.6778C33.5115 46.6614 33.535 60.6454 33.5072 74.6291C33.5115 78.3559 31.9804 82.411 28.5332 84.2345C24.2864 86.4345 19.0374 85.9679 14.8419 83.8584C11.2099 82.0456 7.7438 79.9062 4.17621 77.9641C1.69702 76.6809 -0.017145 73.9994 0.000306541 71.189V28.8178C-0.0256927 26.0611 1.60442 23.4053 4.02128 22.1043C16.4418 15.0948 28.8601 8.08001 41.2785 1.06735Z" fill="#8CC84B" />
                        <path d="M52.1141 30.4179C57.5312 30.0692 63.3304 30.2113 68.2051 32.88C71.9793 34.925 74.0717 39.217 74.1383 43.41C74.0329 43.9756 73.4416 44.2876 72.9017 44.2488C71.33 44.2466 69.7579 44.2701 68.1862 44.2381C67.5195 44.2637 67.132 43.649 67.0483 43.0596C66.5967 41.0534 65.5026 39.0664 63.6143 38.0987C60.7156 36.6474 57.3545 36.7204 54.1937 36.7507C51.8862 36.8732 49.4048 37.073 47.4499 38.4299C45.9491 39.4578 45.4932 41.5327 46.0289 43.2038C46.5339 44.4037 47.919 44.7908 49.0522 45.1477C55.5805 46.8551 62.4985 46.6852 68.9021 48.9322C71.5533 49.8482 74.1468 51.629 75.0543 54.4048C76.2414 58.1252 75.721 62.5721 73.0741 65.5589C70.9276 68.0167 67.8012 69.3544 64.6831 70.0809C60.535 71.0059 56.2302 71.0294 52.0176 70.6187C48.0564 70.1671 43.9343 69.1264 40.8764 66.4275C38.2615 64.157 36.9843 60.6194 37.1111 57.2049C37.1414 56.6283 37.7155 56.2262 38.2679 56.2736C39.8507 56.2608 41.4334 56.2565 43.0162 56.2757C43.6483 56.2305 44.117 56.7768 44.1494 57.3727C44.4415 59.2841 45.1598 61.2903 46.8266 62.4236C50.0434 64.4989 54.0797 64.3568 57.7631 64.4149C60.8146 64.2795 64.2401 64.2386 66.7303 62.2217C68.0441 61.0713 68.4334 59.1467 68.0783 57.4909C67.6937 56.093 66.2313 55.4416 64.9755 55.0156C58.5309 52.977 51.5357 53.7167 45.1534 51.4114C42.5624 50.4957 40.0569 48.7644 39.0614 46.1025C37.6724 42.3351 38.3089 37.6752 41.2333 34.7893C44.0846 31.9187 48.2007 30.8132 52.1141 30.4175V30.4179Z" fill="#8CC84B" />
                    </svg>

                },
                {
                    name: "Javascript",
                    icon: <svg width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M100 0.000488281H0V100H100V0.000488281Z" fill="#F7DF1E" />
                        <path d="M67.1745 78.1259C69.1888 81.4148 71.8094 83.8323 76.4444 83.8323C80.338 83.8323 82.8253 81.8862 82.8253 79.1973C82.8253 75.9751 80.2698 74.8339 75.9841 72.9592L73.6348 71.9513C66.8539 69.0624 62.3491 65.4434 62.3491 57.7926C62.3491 50.745 67.719 45.3799 76.111 45.3799C82.0856 45.3799 86.3809 47.4592 89.4761 52.9037L82.1586 57.6021C80.5475 54.7132 78.8094 53.5751 76.111 53.5751C73.3587 53.5751 71.6142 55.3212 71.6142 57.6021C71.6142 60.4212 73.3602 61.5624 77.392 63.3085L79.7412 64.3148C87.7253 67.7386 92.2333 71.2291 92.2333 79.0767C92.2333 87.537 85.5872 92.1719 76.6618 92.1719C67.9348 92.1719 62.2967 88.0132 59.538 82.5624L67.1745 78.1259ZM33.9793 78.9402C35.4555 81.5592 36.7983 83.7735 40.0269 83.7735C43.1142 83.7735 45.0618 82.5656 45.0618 77.8688V45.9164H54.4586V77.9958C54.4586 87.7259 48.7539 92.1545 40.4269 92.1545C32.9031 92.1545 28.546 88.2608 26.3301 83.5712L33.9793 78.9402Z" fill="black" />
                    </svg>
                },
                {
                    name: "Tailwind CSS",
                    icon: <svg width="100" height="60" viewBox="0 0 100 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M50.0002 0.000488281C36.6665 0.000488281 28.3334 6.66548 24.9996 19.9942C30.0002 13.3292 35.8337 10.8308 42.4999 12.4964C46.3039 13.4468 49.0224 16.2065 52.0323 19.2602C56.9341 24.2345 62.6075 29.9917 75.0009 29.9917C88.3334 29.9917 96.6677 23.3267 100 9.99672C95.0009 16.6617 89.1674 19.1614 82.4999 17.4958C78.6973 16.5454 75.9787 13.7856 72.9689 10.732C68.067 5.75764 62.3924 0.000488281 50.0002 0.000488281ZM24.9996 29.9917C11.6671 29.9917 3.33274 36.6567 0.000244141 49.9866C4.99962 43.3217 10.8331 40.822 17.5006 42.4888C21.3032 43.4405 24.0218 46.199 27.0316 49.2514C31.9335 54.2257 37.6081 59.9841 50.0002 59.9841C63.334 59.9841 71.6671 53.3191 75.0009 39.9892C70.0002 46.6542 64.1668 49.1538 57.5006 47.487C53.6966 46.5379 50.9781 43.7768 47.9682 40.7244C43.0664 35.7501 37.393 29.9917 24.9996 29.9917Z" fill="#06B6D4" />
                    </svg>
                },
                {
                    name: "Figma",
                    icon: <svg width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M33.3334 100.001C42.5334 100.001 50.0001 92.5339 50.0001 83.3339V66.6672H33.3334C24.1334 66.6672 16.6667 74.1339 16.6667 83.3339C16.6667 92.5339 24.1334 100.001 33.3334 100.001Z" fill="#0ACF83" />
                        <path d="M16.6667 50.0004C16.6667 40.8004 24.1334 33.3337 33.3334 33.3337H50.0001V66.667H33.3334C24.1334 66.667 16.6667 59.2004 16.6667 50.0004Z" fill="#A259FF" />
                        <path d="M16.6667 16.6671C16.6667 7.46715 24.1334 0.000488281 33.3334 0.000488281H50.0001V33.3338H33.3334C24.1334 33.3338 16.6667 25.8671 16.6667 16.6671Z" fill="#F24E1E" />
                        <path d="M50 0.000488281H66.6667C75.8667 0.000488281 83.3334 7.46715 83.3334 16.6671C83.3334 25.8671 75.8667 33.3338 66.6667 33.3338H50V0.000488281Z" fill="#FF7262" />
                        <path d="M83.3334 50.0004C83.3334 59.2004 75.8667 66.667 66.6667 66.667C57.4667 66.667 50 59.2004 50 50.0004C50 40.8004 57.4667 33.3337 66.6667 33.3337C75.8667 33.3337 83.3334 40.8004 83.3334 50.0004Z" fill="#1ABCFE" />
                    </svg>

                },
                {
                    name: "Django",
                    icon: <>
                        <path d="M35.2064 0.000488281H51.5478V74.8984C43.1774 76.4817 37.0154 77.1047 30.3483 77.1047C10.3895 77.0838 0 68.1672 0 51.0422C0 34.5422 11.0204 23.8338 28.0979 23.8338C30.7479 23.8338 32.7668 24.0423 35.2064 24.6671V0.000488281ZM35.779 38.1646C33.8651 37.5395 32.288 37.3312 30.2688 37.3312C22.0037 37.3312 17.2296 42.3728 17.2296 51.2081C17.2296 59.8103 21.7936 64.5602 30.1639 64.5602C31.9724 64.5602 33.4446 64.4584 35.779 64.1458V38.1646Z" fill="#2BA977" />
                        <path d="M78.2706 25.8446V63.345C78.2706 76.2594 77.3033 82.4698 74.464 87.8242C71.8139 92.9718 68.3228 96.2178 61.1091 99.8031L45.9456 92.6575C53.1593 89.305 56.6503 86.3427 58.8799 81.822C61.214 77.1991 61.9501 71.8447 61.9501 57.7615V25.8449H78.2706V25.8446ZM60.3034 0.000488281H76.6447V16.6047H60.3034V0.000488281Z" fill="#2BA977" />
                    </>

                }
            ]


        },
        {
            title: "Learning",
            technologies: [
                {
                    name: "Threejs",
                    icon: <>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M26.7016 102L2 2.00049L101.023 30.5097L26.7016 102Z" stroke="white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M51.4929 16.2579L63.8349 66.2728L14.3511 52.0136L51.4929 16.2579Z" stroke=" white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M39.2145 58.7971L33.0844 33.9514L57.6688 41.0087L39.2145 58.7971Z" stroke="white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M26.952 9.13611L33.0821 33.9818L8.49768 26.9245L26.952 9.13611Z" stroke="white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M76.1186 23.2992L82.2487 48.1449L57.6643 41.0876L76.1186 23.2992Z" stroke="white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M39.2173 58.8142L45.3474 83.6599L20.7629 76.6026L39.2173 58.8142Z" stroke="white" stroke-width="2.2865" stroke-miterlimit="10" stroke-linejoin="round" />
                    </>

                },

                {
                    name: "Flutter",
                    icon: 
                    // <svg width="81" height="100" viewBox="0 0 81 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <>
                        <path d="M15.3989 65.3954L0 49.9964L50 0.000488281H80.7938L15.3989 65.3954ZM80.7938 46.1366H50L38.465 57.6717L53.8639 73.0706" fill="#42A5F5" fill-opacity="0.8" />
                        <path d="M38.4651 88.4654L50.0001 100H80.794L53.864 73.0706" fill="#0D47A1" />
                        <path d="M23.1025 73.0787L38.4772 57.7L53.8519 73.0746L38.4772 88.4533L23.1025 73.0787Z" fill="#42A5F5" />
                        <path d="M38.4771 88.4533L53.8517 73.0786L55.9979 75.2248L40.6232 90.5994L38.4771 88.4533Z" fill="url(#paint0_linear_790_791)" />
                        <path d="M38.4651 88.4653L61.3129 80.5719L53.864 73.0664" fill="url(#paint1_linear_790_791)" />
                        <defs>
                            <linearGradient id="paint0_linear_790_791" x1="46.1673" y1="80.7631" x2="48.3136" y2="82.9094" gradientUnits="userSpaceOnUse">
                                <stop offset="0.2" stop-opacity="0.15" />
                                <stop offset="0.85" stop-color="#616161" stop-opacity="0.01" />
                            </linearGradient>
                            <linearGradient id="paint1_linear_790_791" x1="38.4705" y1="80.7663" x2="61.318" y2="80.7663" gradientUnits="userSpaceOnUse">
                                <stop offset="0.2" stop-opacity="0.55" />
                                <stop offset="0.85" stop-color="#616161" stop-opacity="0.01" />
                            </linearGradient>
                        </defs>
                        </>
                    // </svg>
                },
                {
                    name: "Ionic",
                    icon: <>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M95.1022 28.2998L95.5103 29.2522C98.5034 35.8508 100 42.7896 100 50.0005C100 77.5514 77.5509 100 50 100C22.449 100 0 77.5514 0 50.0005C0 22.4495 22.449 0.000488281 50 0.000488281C58.0953 0.000488281 65.7822 1.90525 72.8572 5.44268L73.8094 5.91886L72.9931 6.59911C70.9525 8.23177 69.3878 10.2726 68.3675 12.6536L68.0953 13.3338L67.4831 13.0617C61.9728 10.4767 56.1225 9.11615 50 9.11615C27.415 9.11615 9.11566 27.4155 9.11566 50.0005C9.11566 72.5855 27.415 90.8849 50 90.8849C72.585 90.8849 90.8844 72.5174 90.8844 50.0005C90.8844 44.6264 89.8641 39.3202 87.755 34.3542L87.4831 33.6739L88.1631 33.4017C90.5441 32.5174 92.7209 31.0209 94.4219 29.1161L95.1022 28.2998ZM82.7209 29.2523C88.4694 29.2523 93.1294 24.5924 93.1294 18.8441C93.1294 13.0958 88.4694 8.43593 82.7209 8.43593C76.9728 8.43593 72.3128 13.0958 72.3128 18.8441C72.3128 24.5924 76.9728 29.2523 82.7209 29.2523ZM49.9997 27.2115C37.4147 27.2115 27.2106 37.4155 27.2106 50.0008C27.2106 62.5858 37.4147 72.7899 49.9997 72.7899C62.5847 72.7899 72.7887 62.5858 72.7887 50.0008C72.7887 37.4155 62.5847 27.2115 49.9997 27.2115Z" fill="#3880FF" />
                    </>

                },
                // {
                //     name: "Laravel",
                //     icon: <img src="/path/to/laravel-icon.svg" alt="Laravel" />
                // },
                // {
                //     name: "Symfony",
                //     icon: <img src="/path/to/symfony-icon.svg" alt="Symfony" />
                // }
            ]
        }
    ]
    return (
        <div className="w-full mt-5 md:flex border  border-border gap-2">
            <div className="skills flex-[2] border-r border-border ">

                <h2 className="text-2xl font-bold "> My Skills </h2>
                <div className="skills-content flex  justify-between">
                    {technos.map((category, index) => (
                        category.title === "Skilled Technos" ? (
                            <div key={index} className="mb-5">
                                <h3 className="text-xl font-semibold mb-3">{category.title}</h3>
                                <div className="grid grid-cols-2 md:grid-cols-6 lg:grid-cols-3 gap-2">
                                    {category.technologies.map((tech: any, techIndex: any) => (
                                        <div key={techIndex} className="flex  gap-2 items-center">
                                            <div className="w-5 h-5 mb-2">
                                                <svg width="25" height="25" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    {tech.icon}
                                                </svg>
                                            </div>
                                            <p className="text-center">{tech.name}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ) : (
                            <div key={index} className="mb-5 p-2">
                                <h3 className="text-xl font-semibold mb-3">{category.title}</h3>
                                <div className="grid grid-cols-2 md:grid-cols-6 lg:grid-cols-1 gap-2">
                                    {category.technologies.map((tech: any, techIndex: any) => (
                                        <div key={techIndex} className="flex  gap-2 items-center">
                                            <div className="w-5 h-5 mb-2">
                                                <svg width="25" height="25" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                {tech.icon}
                                                </svg>
                                                </div>
                                            <p className="text-center">{tech.name}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        )
                    ))}

                </div>
                

            </div>
            <div className="Mywork flex-1">
                    <h2 className="text-2xl font-bold"> My Works </h2>


                </div>


        </div>
    );
};
